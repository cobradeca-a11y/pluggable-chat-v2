...
async def get_current_user_id(authorization: str = Header(None)) -> str:
    ...
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail={"error_type": "session_expired", "message": "Token de autenticação ausente"})
    ...
    try:
        ...
        if not res or not res.user:
            raise HTTPException(status_code=401, detail={"error_type": "session_expired", "message": "Sessão expirada ou inválida"})
        return res.user.id
